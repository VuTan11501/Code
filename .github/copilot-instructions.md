...
3. **Refactor trước khi tạo mới**: trước khi viết 1 component CSS mới, grep `style.css` xem đã có primitive tương tự chưa. Nếu có → dùng/extend. Nếu phải tạo mới → đặt tên + tokens nhất quán với existing.
   ...
   **🔘 Card action-row buttons — quy ước CỨNG (TOÀN PROJECT, mọi page)**: mọi nút trong hàng action của 1 card (header/footer/toolbar — Settings, Dashboard, OT, Timesheet, Schedule…) PHẢI dùng `class="btn btn-outline sm"` (giống "Export Vault" / "Import Vault" / "Sync DokoKin" / "Refresh" / "Run now" / "Edit"). Biến thể destructive (Delete / Cancel / Disable / Clear log) thêm `danger-outline` → `class="btn btn-outline sm danger-outline"`.
   - ⛔ KHÔNG dùng `btn-secondary`/`btn-destructive` (CẢ HAI đều KHÔNG có CSS → render thành bare grey), bare `btn`/`btn sm` (nền xám đặc), `btn danger sm` (đỏ-fill), hay `btn-ghost` cho các hàng action này — sẽ lệch size + màu so với phần còn lại.
   - ⛔ KHÔNG ghép `btn-icon` với `btn-outline` cho icon-only delete — `.btn-icon` set `background:var(--secondary)` đè mất nền trong suốt. Icon-only action button vẫn dùng `btn btn-outline sm` (+`danger-outline`) không kèm `btn-icon`.
   - ✅ Nút **stateful toggle** (vd proxy Enable/Disable) phải đổi cả label + class trong render fn: active → `danger-outline` "Disable", off → bỏ `danger-outline` "Enable". KHÔNG để 1 label tĩnh không phản ánh state.
   - Ngoại lệ hợp lệ (GIỮ nguyên, KHÔNG đổi sang outline):
     - **Primary CTA** nổi bật: `btn primary` (Add OT, Trigger, Add Schedule, Sync (timesheet empty-state), Suggest schedule…).
     - **Icon-nav** prev/next tháng: `btn sm btn-ghost btn-icon` (chevron) — minimal có chủ đích.
     - **Modal footer** Cancel/Save pairs + `modal-close` ghost: theo convention modal riêng.
     - **Global header** Lock: `btn danger sm` (chrome chung, affordance đỏ cố ý).
