// ═══════════════════════════════════════════════════
//  ICONS — Lucide-style SVG line icons
//  Usage: ICON.calendar() returns SVG string
//         ICON.calendar(20) custom size
// ═══════════════════════════════════════════════════
const ICON_PATHS = {
  // Brand / status
  ...
  swapHoriz:   '<path d="M7 7 L3 12 L7 17"/><path d="M17 7 L21 12 L17 17"/><path d="M3 12h18"/>',
...
function ICON(name, size = 16, extraClass = '') {
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="icon${cls}" aria-hidden="true">${path}</svg>`;
}
// Shortcut: ICON.name(size)
for (const key of Object.keys(ICON_PATHS)) {
  ICON[key] = (size = 16, cls = '') => ICON(key, size, cls);
}
...
const _origSetInnerHTML = Element.prototype.__lookupSetter__('innerHTML');
// Provide a global helper for code that updates DOM:
window.refreshIcons = (root) => renderIcons(root || document);
window.ICON = ICON;
